...
func TestList(t *testing.T) {
	...
	total := 1
	mock := &api.MockClient{
		ListStoriesFn: func(ctx context.Context, params *gen.ListStoriesParams) (*api.StoryList, error) {
			return &api.StoryList{Stories: &stories, Total: &total}, nil
		},
	}

	svc := NewService(mock)
	result, err := svc.List(context.Background(), &gen.ListStoriesParams{})
	if err != nil {
...
func TestWriteSection(t *testing.T) {
	...
	mock := &api.MockClient{
		WriteSectionFn: func(ctx context.Context, storyID, sectionID string, req api.UpdateSectionRequest) (json.RawMessage, error) {
			if storyID != "story-1" {
				t.Errorf("expected story ID story-1, got %s", storyID)
			}
			if sectionID != "sec-1" {
			...
			called = true
			return json.RawMessage(`{"section":{"id":"sec-1"},"sections":[],"commitSha":"abc","wordCount":7}`), nil
		},
	}

	svc := NewService(mock)
	req := api.UpdateSectionRequest{Content: &content}
	_, err := svc.WriteSection(context.Background(), "story-1", "sec-1", req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !called {
...
func TestStartNarrationGuard(t *testing.T) {
	narration := func(status string) json.RawMessage {
		return json.RawMessage(`{"narration_id":"n-1","status":"` + status + `"}`)
	}
	cases := []struct {
		name        string
		get         func(ctx context.Context, storyID string) (json.RawMessage, error)
		confirm     bool
		wantStarted bool // api.StartNarration reached
		wantErr     bool
	}{
		{
			name: "no narration proceeds",
		...
		{
			name:        "complete narration blocked without confirm",
			get:         func(ctx context.Context, id string) (json.RawMessage, error) { return narration("complete"), nil },
			confirm:     false,
			wantStarted: false,
			wantErr:     true,
		},
		{
			name:        "complete narration proceeds with confirm",
			get:         func(ctx context.Context, id string) (json.RawMessage, error) { return narration("complete"), nil },
			confirm:     true,
			wantStarted: true,
		},
		{
			name:        "in-flight narration not guarded",
			get:         func(ctx context.Context, id string) (json.RawMessage, error) { return narration("processing"), nil },
			confirm:     false,
			wantStarted: true,
		},
		{
			name:        "error narration blocked without confirm",
			get:         func(ctx context.Context, id string) (json.RawMessage, error) { return narration("error"), nil },
			confirm:     false,
			wantStarted: false,
			wantErr:     true,
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotForce := false
			mock := &api.MockClient{
				GetNarrationFn: tc.get,
				StartNarrationFn: func(ctx context.Context, id, voice string, force bool, emphasis *bool) error {
					started = true
					gotForce = force
					return nil
				},
			}
			err := NewService(mock).StartNarration(context.Background(), "s-1", "Kore", tc.confirm, nil)
			...
			if started != tc.wantStarted {
				t.Errorf("api.StartNarration reached=%v, want %v", started, tc.wantStarted)
			}
			// confirm_replace must forward to the backend as force (#245) so a
			// deliberate replace doesn't 409 narration_exists once #477 deploys.
			if started && gotForce != tc.confirm {
				t.Errorf("forwarded force=%v, want %v (= confirm_replace)", gotForce, tc.confirm)
			}
		})
	}
}

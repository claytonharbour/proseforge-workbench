...
func (s *Service) Send(ctx context.Context, entityType, entityID, agent, perspective, target, content string) (*api.SendRoomMessageResponse, error) {
	s.logger.Info("room.Send", "entityType", entityType, "entityID", entityID, "agent", agent)
	resp, err := s.api.SendRoomMessage(ctx, entityType, entityID, api.SendRoomMessageRequest{
		Agent:       agent,

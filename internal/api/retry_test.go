...
func TestRetryTransport_NoRetryOnNonRetryable5xx(t *testing.T) {
	// A deterministic 500 (e.g. the dup-key reservation insert, forge/proseforge#578)
	// marks itself non-retryable; the client must surface it on the first attempt
	// rather than firing the same doomed request 4 times.
	body := `{"error":"reservation_exists","message":"reservation exists for job","retryable":false}`
	mock := &mockTransport{
		responses: []*http.Response{
			{StatusCode: 500, Body: io.NopCloser(bytes.NewReader([]byte(body)))},
		},
	}
	rt := newRetryTransport(mock, 3, testLogger())
	rt.baseDelay = 1 * time.Millisecond
	...
	if mock.calls != 1 {
		t.Errorf("calls = %d, want 1 (non-retryable 5xx must not retry)", mock.calls)
	}
	// The peeked body must be rebuffered intact for the caller (checkResponse).
	got, _ := io.ReadAll(resp.Body)
	if string(got) != body {
		t.Errorf("body = %q, want %q (peek must rebuffer)", string(got), body)
	}
}

func TestRetryTransport_RetriesWhenRetryableTrue(t *testing.T) {
	// An explicit retryable:true (or any 5xx without a false hint) keeps the
	// existing blanket-retry behavior — backward compatible.
	body := `{"error":"internal_error","retryable":true}`
	resp5xx := func() *http.Response {
		return &http.Response{StatusCode: 500, Body: io.NopCloser(bytes.NewReader([]byte(body)))}
	}
	mock := &mockTransport{
		responses: []*http.Response{resp5xx(), resp5xx(), resp5xx(), resp5xx()},
	}
	rt := newRetryTransport(mock, 3, testLogger())
	rt.baseDelay = 1 * time.Millisecond
	...
	// 1 initial + 3 retries = 4 total.
	if mock.calls != 4 {
		t.Errorf("calls = %d, want 4 (retryable 5xx should exhaust retries)", mock.calls)
	}
}

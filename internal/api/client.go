...
func New(baseURL, apiKey string, opts ...Option) (*Client, error) {
	...
	httpClient := &http.Client{
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			...
			// Go's default changes POST to GET on 301/302 (per RFC 7231),
			// which breaks multipart uploads and other POST endpoints.
			if len(via) > 0 {
	...
	return &Client{raw: raw, apiKey: apiKey, baseURL: baseURL, httpClient: httpClient}, nil
}

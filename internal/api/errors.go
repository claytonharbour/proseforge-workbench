...
func classifyAPIError(e *APIError) *ErrorInfo {
	...
	serverRetryable := retryableFromServer(e.Body)
	...
	case e.StatusCode == 403:
		info.Category = CategoryAuth
		// Keep whatever code and message the server sent. This branch used to
		// recognise exactly one code and replace every other 403 with a generic
		// string — the only status branch that ignored serverMsg (#449).
		//
		// 🛑 The API states the cause and the remedy: "tier_required" / "Story
		// export requires a Scribe subscription or higher", and "downloads_disabled"
		// / "Author has disabled public downloads for this story". The CLI prints
		// the raw body and could always see those; the MCP tools ran through here
		// and could not. Four benches spent a morning concluding that a 403
		// discriminates nothing, from the wrapper's string rather than the API's.
		//
		// ⚠️ An allow-list cannot hold: three of the four codes observed on
		// 2026-08-29 were unknown to it, and a code added upstream is silently
		// flattened with nothing going red here.
		info.Code = codeFromServer(e.Body, "forbidden")
		if info.Code == "members_only" {
			// Kept hand-written: more actionable than the server's wording.
			info.Message = "This story is members-only. Authenticate with a registered account to read it."
		} else {
			info.Message = withFallback(serverMsg, "Access denied. You don't have permission for this operation.")
		}
	...
	// An explicit backend retryable hint overrides the status-based default for
	// server errors. This lets a deterministic 5xx (e.g. the dup-key reservation
	// insert in forge/proseforge#578) declare itself non-retryable so the client
	// surfaces it immediately instead of retrying a request that can never succeed.
	if serverRetryable != nil && info.Category == CategoryServerError {
		info.Retryable = *serverRetryable
	}
...
// wirePartner: none — error envelope, deliberately narrower than HandlersErrorResponse: we add `retryable` and omit its `_debug`/`authenticated_as` diagnostics
type serverErrorBody struct {
	Error   string `json:"error"`
	Message string `json:"message"`
	// Retryable is the backend's explicit hint about whether retrying the same
	// request could succeed. A pointer so an absent field (nil) is distinct from
	// an explicit false: only an explicit false suppresses retries. Deterministic
	// 5xx failures (e.g. a duplicate-key reservation insert, forge/proseforge#578)
	// set this false so the client stops hammering a request that can never succeed.
...
// retryableFromServer extracts the explicit "retryable" hint from a JSON error
// body. Returns nil when the body is empty, unparseable, or omits the field —
// callers treat nil as "no hint, fall back to status-based policy".
func retryableFromServer(body string) *bool {
	if body == "" {
		return nil
	}
	var seb serverErrorBody

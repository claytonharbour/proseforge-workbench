...
func (c *Client) RequestReviewer(ctx context.Context, req CreateReviewerRequestReq) error {
	...
	if err != nil {
		return fmt.Errorf("requesting reviewer: %w", err)
	}
...
func (c *Client) RespondToReviewerRequest(ctx context.Context, requestID string, req RespondToReviewerReq) error {
	...
	if err != nil {
		return fmt.Errorf("responding to reviewer request %s: %w", requestID, err)
	}
	defer resp.Body.Close()
...
func (c *Client) ListAvailableReviewers(ctx context.Context) (json.RawMessage, error) {
	resp, err := c.raw.GetReviewersAvailable(ctx)
	if err != nil {
		return nil, fmt.Errorf("listing available reviewers: %w", err)
	}
	defer resp.Body.Close()
...
func (c *Client) ListMyReviewers(ctx context.Context) ([]Reviewer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("listing my reviewers: %w", err)
	}
	defer resp.Body.Close()

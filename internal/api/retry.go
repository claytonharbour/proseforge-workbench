...
func (t *retryTransport) shouldRetry(resp *http.Response, err error) (bool, string) {
	...
	case 500, 502, 503, 504:
		// A 5xx is retryable by default, but the backend can override that with
		// an explicit {"retryable": false} in the body. Deterministic failures
		// like the dup-key reservation insert (forge/proseforge#578) can never
		// succeed on retry, so honor the hint and surface them immediately
		// instead of firing the same doomed request 1+maxRetries times.
		if serverSaysNoRetry(resp) {
...
// serverSaysNoRetry peeks a response body for an explicit {"retryable": false}
// hint and rebuffers the body so downstream readers (checkResponse) still see it
// intact. Returns true only when the backend explicitly marks the error
// non-retryable; an absent hint, unreadable body, or read error all fall back to
// the status-based retry default (returns false → caller keeps retrying).
func serverSaysNoRetry(resp *http.Response) bool {
	...
	hint := retryableFromServer(string(body))

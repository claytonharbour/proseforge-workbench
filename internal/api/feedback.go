...
func (c *Client) GetFeedbackReview(ctx context.Context, storyID, reviewID string, include ...string) (*FeedbackReview, error) {
	params := &gen.GetStoryIdFeedbackReviewsReviewIdParams{}
	hasItems := false
	if len(include) > 0 {
		inc := include[0]
		params.Include = &inc
	...
	resp, err := c.raw.GetStoryIdFeedbackReviewsReviewId(ctx, storyID, reviewID, params)
	if err != nil {
...
func (c *Client) GetFeedbackReviewFull(ctx context.Context, storyID, reviewID string) (*FeedbackReviewWithItems, error) {
	items := "items"
	params := &gen.GetStoryIdFeedbackReviewsReviewIdParams{Include: &items}

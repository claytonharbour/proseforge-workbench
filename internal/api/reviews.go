...
func (c *Client) AddReviewer(ctx context.Context, storyID string, req AddReviewerRequest) (*Reviewer, error) {
	resp, err := c.raw.CreateStoryReviewer(ctx, storyID, gen.CreateStoryReviewerJSONRequestBody(req))
	if err != nil {
		return nil, fmt.Errorf("add reviewer to story %s: %w", storyID, err)
	}
...
func (c *Client) ListReviewers(ctx context.Context, storyID string) (*ReviewersList, error) {
	resp, err := c.raw.ListStoryReviewers(ctx, storyID)
	if err != nil {
		return nil, fmt.Errorf("list reviewers for story %s: %w", storyID, err)
	}
	defer resp.Body.Close()
...
func (c *Client) AcceptReview(ctx context.Context, reviewID string) error {
	resp, err := c.raw.AcceptReview(ctx, reviewID)
	if err != nil {
		return fmt.Errorf("accept review %s: %w", reviewID, err)
	}
	defer resp.Body.Close()
...
func (c *Client) DeclineReview(ctx context.Context, reviewID string) error {
	resp, err := c.raw.DeclineReview(ctx, reviewID)
	if err != nil {
		return fmt.Errorf("decline review %s: %w", reviewID, err)
	}
	defer resp.Body.Close()
...
func (c *Client) ApproveStory(ctx context.Context, reviewID string) error {
	resp, err := c.raw.ApproveReview(ctx, reviewID)
	if err != nil {
		return fmt.Errorf("approve review %s: %w", reviewID, err)
	}
	defer resp.Body.Close()
...
func (c *Client) RejectStory(ctx context.Context, reviewID string, req ReviewFeedbackRequest) error {
	resp, err := c.raw.RejectReview(ctx, reviewID, gen.RejectReviewJSONRequestBody(req))
	if err != nil {
		return fmt.Errorf("reject review %s: %w", reviewID, err)
	}
	defer resp.Body.Close()

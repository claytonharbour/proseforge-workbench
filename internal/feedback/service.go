...
// waitForCompletion polls the review status until it reaches "completed" or times out.
...
func (s *Service) waitForCompletion(ctx context.Context, storyID, reviewID string) error {
	...
	for time.Now().Before(deadline) {
		...
		if status == "completed" {
			s.logger.Info("feedback.IncorporateAll", "reviewID", reviewID, "status", "completed")
			return nil
		}

...
func SamplingWarning(observed, maxAge, loopInterval time.Duration) string {
	...
	if loopInterval > spacing {
		return fmt.Sprintf(
			"🛑 --interval %s is coarser than this leg's poll spacing %s. Some outage lengths are "+
				"undetectable at ANY --max-age — a breach can fall entirely between samples. "+
				"Use --interval %s or less.",
	...
	for n := 1; n <= 16; n++ {
		...
		if peak <= maxAge {
			continue // tolerated by design — not a miss
		}
		if excursion := peak - maxAge; excursion < loopInterval {
			return fmt.Sprintf(
				"⚠️ --max-age %s sits %s below the %d-failure peak %s, under the %s sample interval — "+
					"that outage is detected only some of the time. Use %s to tolerate %d deterministically.",
				maxAge, excursion.Round(time.Second), n, peak.Round(time.Second),
				loopInterval, (peak + time.Second).Round(time.Second), n)
		}

...
const (
	...
	// fileSubject records WHAT this leg watches — entity type, entity id and
	// backend — written at arm time.
	//
	// 🛑 A STATE DIR PROVES A LEG RUNS. IT DID NOT PROVE WHAT IT RUNS AGAINST.
	// `room health` and `room roster` could both say "this is polling" while
	// nothing on disk said which room, or which environment. @Sten found a live
	// PROD watcher on their node, 25.5 hours old, and could not establish from
	// any artifact whether it should exist — the running process was the only
	// record of its own subject.
	//
	// ⚑ CLAUDE.md names the gap: "What NOTHING checks: whether a watcher SHOULD
	// exist." That question cannot even be ASKED across a fleet until the
	// subjects are enumerable, and a process list is not an inventory — it dies
	// with the process.
	//
	// ⚠️ Base URL only. It is the environment, never a credential; the token
	// lives in a separate file this never reads.
	fileSubject = "watching"
)
...
type Message struct {
	...
	// ReplyTo is the parent this message threads under (forge/proseforge#1269).
	//
	// 🛑 The queue is the DURABLE record — a field dropped here is lost for good,
	// because the poll that could have carried it will not run again. @Sten found
	// the watcher stripping this, which meant thread context never survived
	// delivery to any consumer.
	ReplyTo string `json:"replyTo,omitempty"`

	// ⛔ THE SAME DEFECT, THREE MORE FIELDS (#471). The comment above was written
	// when ReplyTo was added and it describes a CLASS — but only ReplyTo was
	// fixed, so the API's other three threading fields kept being stripped
	// directly beneath the warning about stripping them. @Smiley found it.
	//
	// ⚠️ replyTo alone answers "what is my parent". It does NOT answer "what
	// thread am I in" (threadRootId — a reply to a reply flattens to the root)
	// or "whose message am I answering" (parentPrincipalId), and principalId is
	// the author's stable id where `agent` is a display name that can repeat.
	//
	// 🛑 And backfill is impossible for the same reason the class comment gives:
	// every line already written is permanently missing these.
	ThreadRootID      string `json:"threadRootId,omitempty"`
	ParentPrincipalID string `json:"parentPrincipalId,omitempty"`
	PrincipalID       string `json:"principalId,omitempty"`

	// ⛔ Reactions are DELIBERATELY NOT carried. The API's own type says why: a
	// reaction is mutable and a stream entry is not. A count frozen at poll time
	// would be wrong by the next tick and would look authoritative in an
	// append-only file. Read them live with `room read`, never from the queue.
}
...
type WatchOptions struct {
	...
	// BaseURL is recorded, never called — the watcher gets its client from the
	// caller. It exists so the state dir can say WHICH ENVIRONMENT this leg
	// polls, which is half of "should this leg exist".
	BaseURL string
	...
	// IncludeQuoted disables the #417 code elision, restoring the server's own
	// matching semantics. Off by default: a routing pattern quoted as code is the
	// SUBJECT of a message, not its address.
	IncludeQuoted bool
...
type WatchResult struct {
	...
	// QuotedOnly counts messages whose ONLY match was inside quoted code (#417).
	// Reported so the suppression is visible — @Wayland's bound was that this fix
	// must not fail silently toward missed messages.
	QuotedOnly   int  `json:"quoted_only,omitempty"`
...
func Watch(ctx context.Context, r RoomReader, opts WatchOptions) (*WatchResult, error) {
	...
	// 🛑 HERE, not in the --loop wrapper. The first version stamped in the CLI's
	// loop setup and a SINGLE-SHOT run never reached it — so cron-driven legs,
	// which are the ones nobody remembers arming, were exactly the ones with no
	// record of what they watch. Beside StampVersion is the one place both paths
	// pass through.
	StampSubject(opts.StateDir, opts.EntityType, opts.EntityID, opts.BaseURL,
		ScopeRecord{Match: opts.Match, From: opts.From, ExcludeFrom: opts.ExcludeFrom, Limit: opts.Limit})
	...
	if !local.Empty() {
		// ⚠️ TWO REASONS a message is dropped here, and they must not be conflated.
		//
		// ServerMissed means "the backend ignored our filter parameters" and
		// prints a loud warning about an old build. QuotedOnly means "this only
		// matched inside a quoted routing pattern" (#417) — an EXPECTED divergence
		// from the server, which does not elide code and never will.
		//
		// 🛑 Counting the second as the first would make every filter discussion
		// in the room accuse the backend of being stale, on every leg in the
		// fleet. Fixing noise by shipping a false alarm is a worse trade.
		kept := msgs[:0]
		for _, m := range msgs {
			switch {
			case local.Allows(m) || (opts.IncludeQuoted && local.AllowsRaw(m)):
				kept = append(kept, m)
			case local.AllowsRaw(m):
				res.QuotedOnly++ // #417: matched only inside quoted code
			default:
				res.ServerMissed++ // the server should have dropped this
			}
		}
		msgs = kept
	}
...
// StampSubject records what this leg watches AND the scope it watches it with,
// so an inventory can be taken from disk rather than from a process listing.
// Overwritten on every arm: a leg re-pointed or re-scoped stops claiming the old
// configuration.
//
// 🛑 THE SCOPE LINE EXISTS BECAUSE A NARROWED LEG PASSES EVERY OTHER CHECK (#406).
// After the 2026-08-23 reboot every bench re-armed from a saved command — the
// pre-crash argv dies with the process — and @Smiley's leg came back NARROWER
// than it went down. Nothing detected it: a narrowed leg is `alive`, `rc=0`,
// advances its cursor, and delivers messages. It just stops delivering some it
// used to.
//
// ⛔ `fetched=N queued=0` (#403) cannot see it either — a narrowed filter yields
// `fetched=8 queued=3`, an entirely ordinary line.
//
// ⚑ @Wayland's distinction is why this is worth a file when a cumulative counter
// was not: a config snapshot is written ONCE PER ARM, so its staleness IS the
// signal. A counter's reset was a silent defect; this file being out of date
// means the leg was re-armed, which is exactly what you want to know.
func StampSubject(dir, entityType, entityID, baseURL string, scope ScopeRecord) {
	if dir == "" || entityID == "" {
		return
	}
	_ = writeFileAtomic(filepath.Join(dir, fileSubject),
		fmt.Sprintf("%s %s %s\n%s\n", entityType, entityID, baseURL, scope.String()))
}

// ScopeRecord is the filter set a leg was armed with — the thing no other
// artifact keeps.
...
// String renders one stable line. ⚠️ Order is the caller's, NOT sorted: two
// arms that differ only in flag order are the same scope, and normalising here
// would hide a genuine reordering if the semantics ever depend on it.
func (s ScopeRecord) String() string {
	...
	return fmt.Sprintf("match=%s from=%s exclude-from=%s limit=%d",
		j(s.Match), j(s.From), j(s.ExcludeFrom), s.Limit)
}

// Subject reads back what StampSubject wrote. An empty return means the leg was
// armed before this existed — NOT that it watches nothing, which is why the
// caller prints a dash rather than an assertion.
func Subject(dir string) (entityType, entityID, baseURL, scope string) {
	raw := readTrimmed(filepath.Join(dir, fileSubject))
	...
	// ⚠️ Absent on a leg armed before the scope line existed. Returned empty so
	// the caller prints nothing rather than asserting "no filters" — which is a
	// different and much more alarming claim.
	if len(lines) > 1 {
		scope = strings.TrimSpace(lines[1])
	}
	return
}

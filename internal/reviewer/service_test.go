...
func TestRequestReviewer(t *testing.T) {
	...
	mock := &api.MockClient{
		RequestReviewerFn: func(ctx context.Context, req api.CreateReviewerRequestReq) error {
			if req.FriendId == nil || *req.FriendId != "user-42" {
				t.Errorf("expected reviewer ID user-42, got %v", req.FriendId)
			}
			called = true
			return nil
		},
	...
	svc := NewService(mock)
	req := api.CreateReviewerRequestReq{FriendId: strPtr("user-42")}

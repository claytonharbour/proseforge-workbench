...
func (s *Service) ListPending(ctx context.Context, params *gen.GetReviewsPendingParams) (*api.PendingReviews, error) {
	s.logger.Info("review.ListPending")
...
func (s *Service) AddReviewer(ctx context.Context, storyID string, req api.AddReviewerRequest) (*api.Reviewer, error) {
	s.logger.Info("review.AddReviewer", "storyID", storyID)
...
// ActiveReview returns the currently running feedback review for a story, if any.
func (s *Service) ActiveReview(ctx context.Context, storyID string) (*api.FeedbackReview, error) {
	s.logger.Info("review.ActiveReview", "storyID", storyID)
	list, err := s.api.GetFeedbackReviews(ctx, storyID)
	if err != nil {
		return nil, err
	}
	if list.Reviews == nil {
	...
	for _, r := range *list.Reviews {
		if r.Status != nil && *r.Status == api.ReviewStatusRunning {
			return &r, nil
		}
	}
	return nil, nil

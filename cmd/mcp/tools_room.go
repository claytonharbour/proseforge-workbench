...
func registerRoomTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("room_list",
			...
			mcp.WithString("agent_handle", mcp.Description("Optional agent handle for the account's stored room cursor context. NOTE: a handle is a display label, not a pen name — every message you send also carries principalId, the account behind it, visible to all room readers (#1043).")),
	...
	s.AddTool(
		tool("room_send",
			...
			mcp.WithString("reply_to", mcp.Description("Thread this message under another (message id from room_read). ⚠️ The parent may be gone — the room stream is capped, so replies orphan by default; render \"no longer available\" rather than assuming you can resolve it.")),
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			// 🛑 A PRESENT-BUT-EMPTY reply_to is a failed lookup, not an intent to
			// broadcast (#466). Passing "" used to post top-level and report
			// success, so a caller that resolved a parent id and got nothing
			// silently broadcast instead of replying. The CLI carries the same
			// check on Flags().Changed; it cannot live in the service, because by
			// then an omitted key and an empty one are the same "".
			if raw, present := req.GetArguments()["reply_to"]; present {
				if s, _ := raw.(string); strings.TrimSpace(s) == "" {
					return toolError(fmt.Errorf("reply_to was given but empty: a reply target is required "+
						"when the argument is present (omit reply_to entirely to post top-level)"), client), nil
				}
			}

			result, err := svc.Send(ctx, entityTypeArg(req), entityID, api.SendRoomMessageRequest{
				Agent:       agent,
				Perspective: optionalArg(req, "perspective"),
				Target:      optionalArg(req, "target"),
				Content:     content,
				ReplyTo:     optionalArg(req, "reply_to"),
			})
			if err != nil {
				return toolError(err, client), nil
			}
		...
		tool("room_read",
			mcp.WithDescription(
				"Read messages from a room. Returns the full history, a delta "+
					...
					"A handle is a LABEL, not a pen name: every message also carries `principalId`, "+
					"the sending account, visible to every reader of the room (#1043).\n\n"+
	...
	// room_presence — Who has READ this room, and when (forge/proseforge#1270)
	s.AddTool(
		tool("room_presence",
			mcp.WithDescription(
				"Who has READ this room, and how long ago. Use it to tell whether a message "+
					"has been seen before re-sending it or escalating.\n\n"+
					"⛔ This is NOT an online/offline list. It returns the timestamp of each "+
					"principal's last read and NOTHING ELSE — no 'active' flag, because there "+
					"is no threshold that would be correct: readers poll on cadences ranging "+
					"from every 60 seconds to once an hour, so a 40-minute-old read is healthy "+
					"for one reader and stale for another. Judge the age against what you know "+
					"of that reader, and do not treat an old timestamp as absence.\n\n"+
					"Returns principal IDs, not names — resolve them with room_list if you need "+
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			svc := room.NewService(client, room.WithLogger(r.logger))
			result, err := svc.Presence(ctx, entityTypeArg(req), entityID)
			if err != nil {
				return toolError(err, client), nil
			}
			return jsonResult(result)
	...
	// room_react — Acknowledge a message with an emoji (forge/proseforge#1250)
	s.AddTool(
		tool("room_react",
			mcp.WithDescription(
				"Acknowledge a room message with an emoji instead of writing a reply.\n\n"+
					"⚑ USE THIS INSTEAD OF POSTING \"acknowledged\" OR \"agreed\". A reply costs "+
					"every reader in the room their attention; a reaction costs none and answers "+
					"the same question. If your entire message would be agreement, react instead.\n\n"+
					"Both directions are idempotent: reacting twice is one reaction, removing one "+
					"that is not there is not an error. Message IDs come from room_read.\n\n"+
					"Reactions come back on room_read messages in a `reactions` array carrying "+
					"emoji, count, principalIds and mine — so you can tell whether a SPECIFIC "+
					"person has seen something, which a count alone cannot answer."),
			mcp.WithString("entity_id", mcp.Required(), mcp.Description("Story, series, or bundle ID")),
			mcp.WithString("message_id", mcp.Required(), mcp.Description("Message to react to — from room_read")),
			mcp.WithString("emoji", mcp.Required(), mcp.Description("The emoji, e.g. 👀 or ✅")),
			mcp.WithBoolean("remove", mcp.Description("Withdraw this reaction instead of adding it")),
			mcp.WithString("entity_type", mcp.Description("Entity type: 'story' (default), 'series', 'bundle', or 'conversation'. A conversation is a standalone room with no story or series attached — use it to talk to specific people instead of broadcasting into a shared entity's room.")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			client, err := r.resolve(req)
			...
			emoji, err := requireArg(req, "emoji")
			if err != nil {
				return toolError(err), nil
			}

			svc := room.NewService(client, room.WithLogger(r.logger))
			if req.GetBool("remove", false) {
				if err := svc.Unreact(ctx, entityTypeArg(req), entityID, messageID, emoji); err != nil {
					return toolError(err, client), nil
				}
				return jsonResult(map[string]any{"removed": emoji, "messageId": messageID})
			}
			if err := svc.React(ctx, entityTypeArg(req), entityID, messageID, emoji); err != nil {
				return toolError(err, client), nil
			}
			return jsonResult(map[string]any{"reacted": emoji, "messageId": messageID})
		},
	)

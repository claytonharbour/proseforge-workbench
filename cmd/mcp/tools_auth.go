...
func registerAuthTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("whoami",
			mcp.WithDescription("Report which account the workbench is authenticated as — email, id, and admin flag. Use it to confirm your identity before writing, and to diagnose a 403: with one server shared by several agents, a call that omits credentials_file acts as the server's default account, not you."),

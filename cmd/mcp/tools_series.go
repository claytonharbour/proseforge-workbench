...
func registerSeriesTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("series_world_get",
			mcp.WithDescription("Get the world overview document for a series. The world doc is the canonical home for series direction: setting, tone, core truths, revision doctrine, anti-patterns. Read this before any writing or revision work."),
			mcp.WithString("series_id", mcp.Required(), mcp.Description("Series ID")),
			mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: mcp.ToBoolPtr(true)}),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	s.AddTool(
		tool("series_world_update",
			mcp.WithDescription("Update the world overview document for a series. This is the canonical home for series direction — setting, tone, core truths, revision doctrine, and anti-patterns. When a room debate produces a series-level decision, harvest it here."),
			mcp.WithString("series_id", mcp.Required(), mcp.Description("Series ID")),
			mcp.WithString("content", mcp.Required(), mcp.Description("World overview content (freeform markdown). Recommended sections: # The Setting, ## Tone, ## Core Truths, ## Themes, ## Prose Style Rules, ## Anti-Patterns (DO NOT REPEAT).")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			client, err := r.resolve(req)
			if err != nil {

...
func TestCollaborationMutationErrorsPreserveBackendContract(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// #449: was asserting the generic string, i.e. certifying that the
			// server's cause got discarded. The 409 case above keeps its message;
			// 403 was the only status that did not.
			wantCode: "forbidden", wantMsg: "not your contribution",
		},
		{

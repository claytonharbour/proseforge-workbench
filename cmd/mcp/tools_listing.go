...
func registerListingTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			svc := listing.NewService(client, listing.WithLogger(r.logger))
			result, err := svc.List(ctx, storyID)
			if err != nil {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			svc := listing.NewService(client, listing.WithLogger(r.logger))
			result, err := svc.Create(ctx, storyID, store, format, status, listingURL)
			if err != nil {
				return toolError(err, client), nil
			}
			return jsonResult(result)
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			svc := listing.NewService(client, listing.WithLogger(r.logger))
			result, err := svc.Update(ctx, listingID,
				optionalArg(req, "store"), optionalArg(req, "format"),
				optionalArg(req, "status"), optionalArg(req, "listing_url"))
			if err != nil {
				return toolError(err, client), nil
			}
			return jsonResult(result)
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			svc := listing.NewService(client, listing.WithLogger(r.logger))
			if err := svc.Delete(ctx, listingID); err != nil {
				return toolError(err, client), nil
			}

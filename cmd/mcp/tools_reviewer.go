...
func registerReviewerTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			// reviewer_add uses the review service (AddReviewer is a review operation),
			// but exposed via reviewer tools for MCP grouping. We use the API client directly
			// since this bridges review and reviewer concerns.
			...
			result, err := client.AddReviewer(ctx, storyID, reqBody)
			if err != nil {

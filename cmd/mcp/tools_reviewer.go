...
func registerReviewerTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("reviewer_add",
			mcp.WithDescription("Add a reviewer to a story (author operation). Requires story_id. Use reviewer_available to find reviewer IDs."),
	...
	s.AddTool(
		tool("reviewer_available",
			mcp.WithDescription("List users available as reviewers. Returns reviewer IDs for use with reviewer_add."),

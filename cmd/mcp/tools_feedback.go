...
func registerFeedbackTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("feedback_list",
			mcp.WithDescription("List feedback reviews for a story"),
	...
	s.AddTool(
		tool("feedback_get",
			mcp.WithDescription("Get feedback review details with all feedback items inline"),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
	...
	s.AddTool(
		tool("feedback_suggestions",
			mcp.WithDescription("List suggestions for a feedback review"),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
			mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: mcp.ToBoolPtr(true)}),
		),
	...
	s.AddTool(
		tool("feedback_diff",
			mcp.WithDescription("Get diff of suggested changes for a feedback review"),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
			mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: mcp.ToBoolPtr(true)}),
		),
	...
	s.AddTool(
		tool("feedback_item_add",
			mcp.WithDescription("Add a feedback item to a review (replacement, suggestion, strength, opportunity, or context)"),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
	...
	s.AddTool(
		tool("feedback_section_update",
			mcp.WithDescription("Rewrite a section's content in the feedback branch. Use for structural fixes: cutting bloat, fixing POV, eliminating duplicates."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
	...
	s.AddTool(
		tool("feedback_submit",
			mcp.WithDescription("Submit a review, marking it as ready for the author"),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			client, err := r.resolve(req)
	...
	s.AddTool(
		tool("feedback_incorporate",
			mcp.WithDescription("Incorporate feedback changes into the story (author operation)"),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),

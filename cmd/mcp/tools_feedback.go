...
func registerFeedbackTools(s *server.MCPServer, r *clientResolver) {
	...
	s.AddTool(
		tool("feedback_list",
			mcp.WithDescription("List all feedback reviews (AI and human) for a story. Each has status: pending, analyzing, ready, incorporated, rejected."),
	...
	s.AddTool(
		tool("feedback_get",
			mcp.WithDescription("Get feedback review metadata (thin by default). Returns all feedback items inline. Use feedback_suggestions for conflict-aware grouped view."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
	...
	s.AddTool(
		tool("feedback_suggestions",
			mcp.WithDescription("Suggestions grouped by section with conflict detection and canApply indicators. Use to decide which changes to incorporate."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
			mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: mcp.ToBoolPtr(true)}),
		),
	...
	s.AddTool(
		tool("feedback_diff",
			mcp.WithDescription("File-by-file diffs showing what the review would change. Includes conflict detection against current content."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),
			mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: mcp.ToBoolPtr(true)}),
		),
	...
	s.AddTool(
		tool("feedback_create",
			mcp.WithDescription("Trigger the built-in AI Story Coach for a story. Owner-only. NOT for human reviewers — use reviewer_add instead."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			client, err := r.resolve(req)
	...
	s.AddTool(
		tool("feedback_incorporate",
			mcp.WithDescription("Apply feedback changes to the story. Author operation. Mode 'file' for file-level or 'suggestion' for individual items."),
			mcp.WithString("story_id", mcp.Required(), mcp.Description("Story ID")),
			mcp.WithString("review_id", mcp.Required(), mcp.Description("Review ID")),

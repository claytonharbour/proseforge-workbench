...
func TestEveryEntityTypeDescriptionNamesEverySupportedType(t *testing.T) {
	...
	for _, name := range []string{
		...
		"room_cursor_set", "room_status", "room_archive", "room_unarchive",
	} {
		tool := s.GetTool(name)
		if tool == nil {
	...
	if checked != 8 {
		t.Fatalf("checked %d entity_type descriptions, expected 8", checked)
	}
	if len(seen) != 1 {
		t.Errorf("the 8 entity_type descriptions are not identical (%d distinct variants) — "+

...
func newStorySectionWriteCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Update a section's content or name",
		Long:  "Update a section's content or name. Use --stdin to read content from stdin, --content for short text, and --name to rename without rewriting.",
...
func runStorySectionWrite(cmd *cobra.Command, args []string) error {
	...
	req, err := sectionWriteRequest(content, name)
	if err != nil {
		return err
	}

	data, err := svc.WriteSection(cmd.Context(), args[0], args[1], *req)
	if err != nil {
		return err
	}
	if isJSON(cmd) {
...
func sectionWriteRequest(content, name string) (*api.UpdateSectionRequest, error) {
	if strings.TrimSpace(content) == "" && strings.TrimSpace(name) == "" {
		return nil, fmt.Errorf("content or name is required: use --stdin, --content, or --name")
	}

...
gets 0. If you must pipe, take the real code explicitly:
  ...
  out=$(pfw room health "$STATE" --max-age 5m); rc=$?`,
	...
	// #400 (@Vance). A one-shot cannot evaluate the sampling guard, because the
	// guard needs a watchdog cadence and a one-shot has none — so it printed
	// byte-identical output whether a config was exposed or sound, which is the
	// null-instrument shape this command exists to remove.
	//
	// ⛔ NOT --interval. That flag is refused without --loop on purpose (a silent
	// no-op interval looked configured), and overloading it would undo a guard to
	// add one. This asks a hypothetical instead: "if my watchdog ran at X, could
	// it see a breach of this threshold?"
	cmd.Flags().Duration("check-cadence", 0,
		"One-shot only: evaluate this leg's threshold as if a watchdog polled it every X (e.g. 2m), "+
			"and report whether a real breach could fall between samples (#400). Implied by --loop's own --interval.")
...
func runRoomHealth(cmd *cobra.Command, args []string) error {
	...
	if isJSON(cmd) {
		...
		// #400: only when explicitly asked. A one-shot has no cadence of its
		// own, so this is the caller's hypothesis, not an observation.
		if cadence, _ := cmd.Flags().GetDuration("check-cadence"); cadence > 0 {
			iv := time.Duration(l.IntervalSeconds) * time.Second
			switch {
			// 🛑 #401: NO OBSERVED INTERVAL IS NOT A PASS. A leg that just
			// re-exec'd stamps arm and arm-prev together, so the interval reads
			// 0 for exactly one tick — 60s on a demo leg, 30 MINUTES on a prod
			// one, beginning the instant you publish, which is when people
			// check. Eliding it answered a direct question with the same empty
			// string this guard uses for "sound", and I ran four cadences
			// through it and got four clean results on a config the next tick
			// called fragile.
			case iv <= 0:
				fmt.Printf("        ⚑ no observed interval yet (this leg has armed once, "+
					"or just re-exec'd) — cannot evaluate --max-age against a %s cadence "+
					"until the next tick. This is a missing input, NOT a pass.\n", cadence)
			default:
				if w := watcher.SamplingWarning(iv, legAge, cadence); w != "" {
					fmt.Printf("        %s\n", w)
				} else if m := watcher.SamplingMargin(iv, legAge, cadence); m != "" {
					// 🛑 A sound verdict without its margin is the same silence as
					// a fragile one (@Aldric). Asked explicitly ⇒ answer with the
					// number, not with the absence of a complaint.
					fmt.Printf("        ✅ %s\n", m)
				}
			}
		}
...
func runRoomHealthLoop(cmd *cobra.Command, specs []string, maxAge, interval time.Duration) error {
	...
	// #401: separate from `seeded` on purpose — the cadence check can only run
	// once the leg has an observed interval, which is not the same moment.
	cadenceChecked := make(map[string]bool, len(specs))
	...
	return loopUntilSignal(cmd, interval, func(context.Context) error {
		for _, spec := range specs {
			...
			//
			// ⛔ #401: "once per leg" MUST NOT mean "once, whether or not it
			// RAN", and it did — this was inside the `seeded` block. It needs
			// the leg's observed interval, which is absent until a leg has
			// armed twice AND for one tick after every re-exec. Seeding is
			// when that is LIKELIEST, since a watchdog is normally started
			// right after a publish, so the check retired itself permanently
			// for that leg on a coin-flip of timing — and left no trace, since
			// its healthy output is silence either way.
			//
			// ⚑ Gated on its own flag and retried until the input exists.
			if !cadenceChecked[dir] && l.IntervalSeconds > 0 {
				cadenceChecked[dir] = true

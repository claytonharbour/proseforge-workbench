...
// ── forge/proseforge#1159: which envelope key wins ───────────────────────────
//
// 🛑 The friends endpoints carried BOTH `friends` and `reviewers` through the ADD
// phase, with IDENTICAL rows in each. That made the preference order untestable
// by observation: reading the wrong key rendered the right output. REMOVE
// (97a91d94) dropped `reviewers`, so from that point a reversed preference
// renders an EMPTY LIST — not an error, not a warning, just no friends.
//
// ⚠️ The divergent case below is the only one that discriminates. Cases where the
// keys agree pass under either ordering and prove nothing, which is exactly how
// this went unpinned while five other tests in this file stayed green.
func TestEnvelopePrefersFriendsOverReviewers(t *testing.T) {
	row := func(name string) []friendRow { return []friendRow{{Name: name}} }
	...
	for _, tc := range []struct {
		...
		{
			name:     "post-REMOVE: only friends is emitted",
			env:      friendsEnvelope{Friends: row("from-friends")},
			wantName: "from-friends",
			because:  "the server after 97a91d94 — the steady state",
		},
		{
			name:     "pre-ADD server: only reviewers is emitted",
			env:      friendsEnvelope{Reviewers: row("from-reviewers")},
			wantName: "from-reviewers",
			because:  "the fallback must survive for an older backend",
		},
		{
			name:     "BOTH present and DIVERGENT — the discriminating case",
			env:      friendsEnvelope{Friends: row("from-friends"), Reviewers: row("from-reviewers")},
			wantName: "from-friends",
			because:  "reversing the preference passes every other case and fails only this one",
		},
		{
			name:     "requests is the last resort, not a competitor",
			env:      friendsEnvelope{Requests: row("from-requests")},
			wantName: "from-requests",
			because:  "friends requests reuse this envelope",
		},
		{
			name:     "friends present but EMPTY falls through rather than short-circuiting",
			env:      friendsEnvelope{Friends: []friendRow{}, Reviewers: row("from-reviewers")},
			wantName: "from-reviewers",
			because:  "an empty slice is not an answer; a nil check alone would strand the fallback",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := tc.env.rows()
			if len(got) != 1 || got[0].Name != tc.wantName {
				t.Fatalf("rows() = %+v, want a single row named %q\nbecause: %s", got, tc.wantName, tc.because)
			}
		})
	}
}

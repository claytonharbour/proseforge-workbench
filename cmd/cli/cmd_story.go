...
func newStoryCmd() *cobra.Command {
	...
	cmd.AddCommand(
		...
		newStoryMetaCmd(),
		newStoryDeleteCmd(),
		newStoryUpdateVisibilityCmd(),
		newStoryRegenerateCmd(),
		newStoryPublishCmd(),
...
func newStoryListCmd() *cobra.Command {
	...
	cmd.Flags().String("status", "", "Filter by status (pitch, draft, queued, generating, published, failed, or all)")
	cmd.Flags().String("q", "", "Search story titles and text")
	cmd.Flags().String("search", "", "Alias for --q")
	cmd.Flags().String("sort", "", "Sort by date, updated time, or rating (for example date_desc)")
	cmd.Flags().String("user", "", "Filter by user ID")
	cmd.Flags().Bool("narration", false, "Only stories with narration")
	cmd.Flags().Bool("audiobook", false, "Only stories with a completed audiobook")
	cmd.Flags().String("cursor", "", "Continue from a next_cursor value")
...
func storyListParams(cmd *cobra.Command) *gen.ListStoriesParams {
	limit, _ := cmd.Flags().GetInt("limit")
	params := &gen.ListStoriesParams{Limit: &limit}
	...
	if value, _ := cmd.Flags().GetString("user"); value != "" {
		params.UserId = &value
	}
	if cmd.Flags().Changed("narration") {
		value, _ := cmd.Flags().GetBool("narration")
		params.Narration = &value
	}
	if cmd.Flags().Changed("audiobook") {
		value, _ := cmd.Flags().GetBool("audiobook")
		params.Audiobook = &value
	}
...
func newStorySectionGroupCmd() *cobra.Command {
	...
	cmd.AddCommand(
		...
		newStorySectionDeleteCmd(),
		newStorySectionReorderCmd(),
	)
	return cmd
}
...
func runStorySectionList(cmd *cobra.Command, args []string) error {
	...
	// Goes to /story/{id}/sections rather than deriving the list from the
	// story-level read, which 404s for a contributor.
	data, err := svc.ListSections(cmd.Context(), args[0])
	if err != nil {
		return err
	}
...
func runStoryVersions(cmd *cobra.Command, args []string) error {
	...
	params := &gen.ListStoryVersionsParams{}

...
// === image generate-and-wait ===

func newImageGenerateAndWaitCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "generate-and-wait",
		Short: "Generate an AI image and block until it completes (costs 2 credits)",
		Args:  cobra.NoArgs,
		RunE:  runImageGenerateAndWait,
	}
	cmd.Flags().String("story-id", "", "Story ID for context-aware generation")
	cmd.Flags().String("section-id", "", "Section ID for section-specific imagery")
	cmd.Flags().String("prompt", "", "Prompt describing the desired image")
	...
	cmd.Flags().Int("width", 0, "Image width in pixels")
	cmd.Flags().Int("height", 0, "Image height in pixels")
	cmd.Flags().Duration("timeout", 60*time.Second, "Max time to wait for completion")
	return cmd
}

func runImageGenerateAndWait(cmd *cobra.Command, args []string) error {
	svc, err := newStoryService(cmd)
	if err != nil {
		return err
	}
	...
	result, st, err := svc.GenerateImageAndWait(cmd.Context(), req, timeout)
	if err != nil {
		return err
	}
	if st != "" {
...
func newImageUploadCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upload",
		Short: "Upload a pre-made image (BYOAI path, max 10 MiB)",
		RunE:  runImageUpload,
	}
...
func newStoryImagesCmd() *cobra.Command {
	...
	cmd.Flags().Bool("include-sections", false, "Also include images attached to the story's sections")
	return cmd
}
...
func runStoryImages(cmd *cobra.Command, args []string) error {
	...
	includeSections, _ := cmd.Flags().GetBool("include-sections")

	result, err := svc.ListStoryImages(cmd.Context(), args[0], includeSections)
	if err != nil {
		return err
	}

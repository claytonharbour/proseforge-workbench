...
func (r roomReader) ReadSince(ctx context.Context, entityType, entityID, since string, limit int,
	...
	for _, m := range resp.Messages {
		out = append(out, watcher.Message{
			...
			ReplyTo: m.ReplyTo,
			// #471 — carry the whole threading set, not just the parent link.
			ThreadRootID: m.ThreadRootID, ParentPrincipalID: m.ParentPrincipalID,
...
recipe that treated 10 as benign silently ignored outages.`,
	...
	cmd.Flags().Bool("include-quoted", false,
		"Also queue messages whose only --match hit is inside quoted code. Off by default: "+
			"a routing pattern shown as code is the SUBJECT of a message, not its address (#417)")
...
func runRoomWatch(cmd *cobra.Command, args []string) error {
	...
	includeQuoted, _ := cmd.Flags().GetBool("include-quoted")
	...
	// ⚠️ Base URL only, and it is recorded rather than used — the client is
	// already built. It names the ENVIRONMENT, which a room id cannot: a prod
	// watcher and a demo watcher on the same room are different questions.
	baseURL, _ := cmd.Flags().GetString("url")
	if expanded, err := config.ExpandEnvRef(baseURL, "--url"); err == nil {
		baseURL = expanded
	}
	if baseURL == "" {
		baseURL = os.Getenv("PROSEFORGE_URL")
	}
	...
	// ⚑ #417: say what the quoted-pattern filter suppressed. @Wayland's bound on
	// that fix was that it must not fail SILENTLY toward missed messages — so the
	// suppression gets a line, not just a smaller number.
	if res.QuotedOnly > 0 {
		status("%d message(s) matched only inside quoted code and were not queued (#417). "+
			"Use --include-quoted if you need them.\n", res.QuotedOnly)
	}
...
func runWatchLoop(cmd *cobra.Command, r watcher.RoomReader, opts watcher.WatchOptions, interval time.Duration) error {
	...
	// #409: say how this process was WIRED, once, at arm. A document cannot
	// enforce a shell redirect; the process can see one.
	checkOutputStreams(func(format string, a ...any) { tel.warn(format, a...) })
	...
	return loopUntilSignal(cmd, interval, func(ctx context.Context) error {
		...
		queued, fetched := 0, 0
		if res != nil {
			queued, fetched = res.Queued, res.Fetched
		}
		// 🛑 `fetched` AS WELL AS `queued` (#403). `queued=0` has three causes
		// and the line recorded only one of them:
		//
		//	fetched=0 queued=0    the room really was quiet
		//	fetched=7 queued=0    the read worked, YOUR FILTER dropped all of it
		//	fetched=0 queued=0    the server returned a delta you did not expect
		//
		// ⚑ @Sten hit exactly this: asked whether a first arm on 2026-08-23
		// under the pre-#1172 server got full history or an account-cursor
		// delta, the answer was unrecoverable — his queue said 0 because his
		// --match dropped all 7, and nothing had recorded the 7. The value was
		// in WatchResult.Fetched the whole time and this line threw it away.
		//
		// ⚠️ WatchResult already documents why: "Reported separately because
		// silent discarding is how you discover a wrong filter months later."
		// The struct got that right and the telemetry did not use it.
		//
		...
		tel.log("tick=%d rc=%d fetched=%d queued=%d pfw=%s%s",
			tick, rc, fetched, queued, Version, errSuffix(err))
...
func reportWatchResult(cmd *cobra.Command, res *watcher.WatchResult, tel *telemetry) {
	...
	if res.Fetched > res.Kept {
		...
		// #445: WHY, not just how many, and in the FILE.
		//
		// 🛑 A drop REASON must never live only on stderr. Where stderr goes is
		// decided by how the operator armed the leg and pfw cannot observe it:
		// an orphaned leg (parent launchd) inherits whatever the arming shell
		// set — measured /dev/null, 2832 bytes already destroyed — while a
		// harness-parented leg gets a socket that IS captured and then never
		// raised, because a Monitor raises stdout only. Different mechanisms,
		// same outcome: the reason is unavailable afterwards.
		//
		// ⚑ Five benches computed a drop rate from the line above and not one
		// could attribute it. A count whose reason is unrecoverable is what
		// sent four of them to the wrong mechanism.
		//
		// ⚠️ A SEPARATE line, not extra fields on the one above: the fleet
		// already greps `filtered: fetched N, kept M`. Additive cannot break
		// a parser; a wider line can.
		tel.log("drop-reasons: quoted %d, self-skipped %d, server-missed %d",
			res.QuotedOnly, res.SelfSkipped, res.ServerMissed)
	}
	if res.Duplicates > 0 {

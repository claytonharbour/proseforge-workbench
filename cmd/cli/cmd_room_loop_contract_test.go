...
// #400 (@Vance): a one-shot `room health` could not evaluate the sampling guard
// at all — no cadence to compare — so it printed byte-identical output whether
// the config was exposed or sound. That is the null-instrument shape this
// command exists to remove, in the command itself.
//
// ⚑ The test that matters is the PAIR. Asserting only that --check-cadence
// warns would pass against a flag that warns unconditionally; asserting only
// that a bare call is quiet would pass against a flag that does nothing.
func TestOneShotCanAskWhetherItsCadenceCanSeeABreach(t *testing.T) {
	bin := pfwBin(t)
	dir := t.TempDir()
	now := time.Now().UTC()
	// A 60s leg: arm and arm-prev 60s apart, so the observed interval is 60s
	// and the peak spacing is 90s.
	for name, ago := range map[string]time.Duration{
		"health-poll": 5 * time.Second, "health-arm": 5 * time.Second,
		"health-arm-prev": 65 * time.Second,
	} {
	...
	// Exposed: 2m cadence against 90s spacing — no threshold can be safe.
	exposed, _, _ := run(t, bin, "room", "health", dir+":5m", "--check-cadence", "2m")
	if !strings.Contains(exposed, "coarser than this leg's poll spacing") {
		t.Errorf("asked about an exposed cadence and got no warning:\n%s", exposed)
	}

	// Sound: 60s cadence, threshold 15s above the 2-failure peak (4m0s) — the
	// centre of the band where the breach still outlasts a 60s sample.
	//
	// ⛔ This fixture has now been wrong TWICE, and both wrongs were silent.
	// :4m1s cleared the warn while sitting one drift-tick from the hazard.
	// :4m45s — the plain midpoint the room and I agreed on — leaves a 45s
	// breach against a 60s cadence and IS the hazard. A "sound config" fixture
	// is copied by whoever reads it, so getting it wrong ships the wrong config
	// to everyone, twice over.
	sound, _, _ := run(t, bin, "room", "health", dir+":4m15s", "--check-cadence", "60s")
	if strings.Contains(sound, "coarser") || strings.Contains(sound, "sits") {
		t.Errorf("false positive on a sound config — the guard must be trustworthy "+
			"when silent, or asking is pointless:\n%s", sound)
	}
	// 🛑 …and "no complaint" is not an answer. @Aldric: a 3s margin and a 33s
	// margin are indistinguishable silence, so an explicit ask must return the
	// number that lets the operator judge for themselves.
	if !strings.Contains(sound, "margin:") {
		t.Errorf("asked about a sound cadence and got silence instead of a margin:\n%s", sound)
	}

	// ⚠️ UNASKED: a one-shot with no cadence must not invent one. Silence here
	// is honest (the input is absent), not a verdict — and the flag must not
	// change behaviour for every existing caller.
	bare, _, _ := run(t, bin, "room", "health", dir+":5m")
	if strings.Contains(bare, "coarser") || strings.Contains(bare, "sample cadence") {
		t.Errorf("a one-shot with no --check-cadence guessed at one:\n%s", bare)
	}
}
